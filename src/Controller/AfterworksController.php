...
class AfterworksController extends AbstractController
{
    ...
    /**
     * @Route("/api/produit/declinaison/{id}", name="api_produitDeclinaison_get", methods={"GET"})
     */
    public function getDeclinaisonsProduit($id): Response
    {

        $declinaisons = $this->produitRepository->getDeclinaisonsProduit($id);
...
/*            // Enregistrer l'objet $post dans la base de données
    ...
    /**
     * @Route("/api/deleteRubrique/{id}", name="api_deleteRubrique_deleteRubrique", methods={"DELETE"})
     */
    public function deleteRubrique(int $id) : Response {
        // Récupérer le body de la requête dans lequel se trouve
        // les données au format JSON du nouveau post à insérer
        // Mettre sous surveillance une partie du code
        ...
        try {
            ...
            $this->entityManager->flush();
            // Renvoyer une réponse HTTP
            $postJSon = $this->serializer->serialize($rubrique,'json');
            return new JsonResponse($postJSon,Response::HTTP_CREATED,[],true);
        } // Intercepter une éventuelle exception
        catch (NotEncodableValueException $exception) {
            $error = [
    ...
    /**
     * @Route("/api/deleteArticle/{id}", name="api_deleteArticle_deleteArticle", methods={"DELETE"})
     */
    public function deleteArticle(int $id) : Response {
        ...
        try {
            ...
            $this->entityManager->flush();
            // Renvoyer une réponse HTTP
            $postJSon = $this->serializer->serialize($article,'json');
            return new JsonResponse($postJSon,Response::HTTP_CREATED,[],true);
        } // Intercepter une éventuelle exception
        catch (NotEncodableValueException $exception) {
            $error = [
    ...
    /**
     * @Route("/api/addProduitsCommande/{noTable}", name="api_addProduitsCommande_createProd", methods={"POST"})
     */
    public function AddProduitsCommande(Request $request, $noTable): Response
    {
        // Récupérer le body de la requête dans lequel se trouve
        // les données au format JSON du nouveau post à insérer
        // Mettre sous surveillance une partie du code
        try {
            ...
            $commandeCreation = new Commande();
            $commandeCreation->setIdStatut(0);
            $commandeCreation->setDateCommande(new \DateTime('now'));
            $commandeCreation->setNoTable($noTable);
            // Enregistrer l'objet $post dans la base de données
            $this->entityManager->persist($commandeCreation); // Préparer l'ordre INSERT
            $this->entityManager->flush(); // Envoyer l'ordre INSERT vers la base de données

            $idCommande = $commandeCreation->getIdCommande();

            $commande = $request->getContent();


            $array = json_decode($commande);
            //var_dump($array);
            // Parcourir la data
            foreach ($array as $row){
                // Désérialiser le json en un objet de la classe Utilisateur
                $jsonRow = json_encode($row);
                $produitCommande = $this->serializer->deserialize($jsonRow,ProduitCommande::class, "json");
                $produitCommande->setIdCommande($idCommande);
                $produitCommande->setMontantTva(20);


                // Enregistrer l'objet $post dans la base de données
                $this->entityManager->persist($produitCommande); // Préparer l'ordre INSERT
                $this->entityManager->flush(); // Envoyer l'ordre INSERT vers la base de données
            }


            // Renvoyer une réponse HTTP
            $postJSon = $this->serializer->serialize($commandeCreation,'json');
            return new JsonResponse($postJSon,Response::HTTP_CREATED,[],true);
        } // Intercepter une éventuelle exception
        catch (NotEncodableValueException $exception) {
            $error = [
